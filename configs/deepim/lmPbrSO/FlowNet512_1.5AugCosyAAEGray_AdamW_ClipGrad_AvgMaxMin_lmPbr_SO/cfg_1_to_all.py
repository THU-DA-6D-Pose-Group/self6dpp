...
base_cfg_name = "FlowNet512_1.5AugCosyAAEGray_AdamW_ClipGrad_AvgMaxMin_Pbr_01_ape.py"
base_id_obj = "01_ape"
base_obj_name = "ape"
...
def main():
    ...
    for obj_id, obj_name in tqdm(id2obj.items()):
        if obj_name in [
            ...
            "bowl",
            "cup",
        ]:  # NOTE: ignore base_obj and some unwanted objs
            continue
        print(obj_name)
        ...
        # ----------------------------------------------------------------------
        new_cfg_path = osp.join(
            cur_dir,
            base_cfg_name.replace(base_id_obj, f"{obj_id:02d}_{obj_name}"),
        )
