...
base_cfg_name = "resnest50d_AugCosyAAE_BG05_mlBCE_DoubleMask_visib10_ycbvRealKuwPbr_100e_01_02MasterChefCan_bop_test.py"
base_id_obj = "01_02MasterChefCan"
...
def main():
    ...
    for obj_id, obj_name in tqdm(id2obj.items()):
        if obj_name in [base_obj_name]:  # NOTE: ignore base_obj and some unwanted objs
            continue
        print(obj_name)
        this_id_obj = "{:02d}_{}".format(obj_id, name_long2short(obj_name))
        # NOTE: what fields should be updated ---------------------------
        new_cfg_dict = dict(
            _base_="./{}".format(base_cfg_name),
            OUTPUT_DIR=cfg.OUTPUT_DIR.replace(base_id_obj, this_id_obj),
            DATASETS=dict(TRAIN=(f"ycbv_{obj_name}_train_real_aligned_Kuw", f"ycbv_{obj_name}_train_pbr")),
        )
        # ----------------------------------------------------------------------
        new_cfg_path = osp.join(cur_dir, base_cfg_name.replace(base_id_obj, this_id_obj))
