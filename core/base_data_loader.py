...
class Base_DatasetFromList(data.Dataset):
    ...
    def aug_bbox_non_square(self, cfg, bbox_xyxy, im_H, im_W):
        ...
        bh = y2 - y1
        bw = x2 - x1
        if cfg.INPUT.BBOX_AUG_TYPE.lower() == "uniform":
            # different to DZI: scale both w and h
            scale_ratio = 1 + cfg.INPUT.BBOX_AUG_SCALE_RATIO * (2 * np.random.random_sample(2) - 1)  # [1-0.25, 1+0.25]
            shift_ratio = cfg.INPUT.BBOX_AUG_SHIFT_RATIO * (2 * np.random.random_sample(2) - 1)  # [-0.25, 0.25]
        ...
        elif cfg.INPUT.BBOX_AUG_TYPE.lower() == "roi10d":
            ...
            bbox_auged = np.array([x1, y1, x2, y2])
        elif cfg.INPUT.BBOX_AUG_TYPE.lower() == "truncnorm":
    ...
    def _bg_img_paths(self):
        ...
        if bg_type == "coco":
            ...
            assert cfg.INPUT.WITH_BG_DEPTH
            img_paths = []
    ...
    def replace_bg(self, im, im_mask, return_mask=False, truncate_fg=False, with_bg_depth=False, depth_bp=False):
        ...
        if with_bg_depth:
            ...
            depth_factor = cfg.INPUT.BG_DEPTH_FACTOR
        else:
