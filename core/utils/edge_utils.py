...
def test_edge_weight():
    ...
    mask_tensor = torch.tensor(pred_masks, dtype=torch.float32).to(device=device)
    img_tensor = torch.tensor(gt_images).to(device=device)  # NCHW, BGR
...
def test_canny():
    ...
    eroded_mask = (
        mask_erode_th(
            torch.tensor(fg_mask, dtype=torch.float32)[None, None],
            kernel_size=11,
        )
        .detach()
        .cpu()
