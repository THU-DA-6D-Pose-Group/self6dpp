...
class ConvPnPNetCls(nn.Module):
    def __init__(
        ...
        with_extent_feat=True,
    ):
        """Use classification heatmap of xyz
        Args:
        ...
        self.with_extent_feat = with_extent_feat
        ...
        if self.with_extent_feat:
            self.fc1 = nn.Linear(fc_in_dim + 128, 1024)  # NOTE: 128 for extents feature
        else:
    ...
    def forward(self, coor_feat, region=None, extents=None, mask_attention=None):
        ...
        if self.with_extent_feat:
            ...
            x = torch.cat([flat_conv_feat, x_extent], dim=1)
        else:
            x = flat_conv_feat
        #
