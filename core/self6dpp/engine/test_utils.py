...
def eval_cached_results(cfg, output_dir, obj_ids=None):
    ...
    result_names = []
    if "MODEL" in cfg and "DEEPIM" in cfg.MODEL:
        iter_num = cfg.MODEL.DEEPIM.get("N_ITER_TEST", 0)
    else:
        iter_num = cfg.TEST.get("ITER_NUM", 0)
    names = ["iter{}".format(i) for i in range(iter_num + 1)]
    exp_id = cfg.EXP_ID
    # print('exp_id', exp_id)
    for name in names:
