...
def simple_test():
    ...
    losser = SSIM(data_range=1.0).cuda()
...
def training_test():
    ...
    # do not output gif， it is huge，output mkv first and convert to gif using ffmpeg
    video_use_gif = False

    im = cv2.imread("test_img1.jpg", 1)
    t_im = torch.from_numpy(im).cuda().permute(2, 0, 1).float()[None] / 255.0
    ...
    # test ssim
    print("Training SSIM")
    rand_im = torch.randint_like(t_im, 0, 255, dtype=torch.float32) / 255.0
    rand_im.requires_grad = True
    optim = torch.optim.Adam([rand_im], 0.003, eps=1e-8)
    losser = SSIM(data_range=1.0, channel=t_im.shape[1]).cuda()
    ssim_score = 0
    while ssim_score < 0.999:
        ...
        if out_test_video:
            if time.perf_counter() - video_last_time > 1.0 / fps:
                video_last_time = time.perf_counter()
    ...
    rand_im.requires_grad = True
    optim = torch.optim.Adam([rand_im], 0.003, eps=1e-8)
    losser = MS_SSIM(data_range=1.0, channel=t_im.shape[1]).cuda()
    ssim_score = 0
    while ssim_score < 0.999:
        optim.zero_grad()
        loss = losser(rand_im, t_im)
...
def performing_test_ssim():
    print("Performance Testing SSIM")
    import time

    s = SSIM(data_range=1.0).cuda()

    a = torch.randint(0, 255, size=(20, 3, 256, 256), dtype=torch.float32).cuda() / 255.0
    b = a * 0.5
