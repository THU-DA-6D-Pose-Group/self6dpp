...
# ================ add lm single image dataset for debug =======================================
debug_im_ids = {
...
# ================ add lmo single image dataset for debug =======================================
debug_im_ids = {
    "test": {obj: [] for obj in ref.lmo_full.objects},
}
for obj in ref.lm_full.objects:
    for split in ["test"]:
        cur_ann_file = osp.join(DATASETS_ROOT, f"BOP_DATASETS/lmo/image_set/lmo_test.txt")
        ann_files = [cur_ann_file]
        ...
        with open(cur_ann_file, "r") as f:
            for line in f:
                # 2/im_id
                im_ids.append("{}/{}".format(2, int(line.strip("\r\n"))))

        debug_im_ids[split][obj] = im_ids
        for debug_im_id in debug_im_ids[split][obj]:
            name = "lmo_single_{}{}_{}".format(obj, debug_im_id.split("/")[1], split)
            if name not in SPLITS_LM:
                SPLITS_LM[name] = dict(
                    name=name,
                    dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/lmo/"),
