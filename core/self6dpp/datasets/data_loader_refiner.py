...
def build_refiner_augmentation(cfg, is_train):
...
def annotations_to_instances(cfg, annos, image_size, mask_format="bitmask", K=None):
    ...
    if len(annos) and "segmentation" in annos[0]:
        ...
        if mask_format == "polygon":
            ...
            masks = MyBitMasks(
                torch.stack([torch.tensor(x.copy()) for x in segms]),
                cpu_only=True,
            )
...
class DeepIM_DatasetFromList(Base_DatasetFromList):
    ...
    def __init__(
        ...
        """
        self.augmentation = build_refiner_augmentation(cfg, is_train=(split == "train"))
    ...
    def read_data(self, dataset_dict):
        ...
        if self.split != "train":
            ...
            # obj_classes, obj_boxes, [obj_poses]
...
def build_refiner_train_loader(cfg, dataset_names):
...
def build_refiner_test_loader(cfg, dataset_name, train_objs=None):
