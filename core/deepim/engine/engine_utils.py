...
def vis_batch(cfg, batch, phase="train"):
    ...
    for i in range(n_obj):
        ...
        if net_cfg.LOSS_CFG.FLOW_LW > 0 and phase == 'train':
        ...
        if net_cfg.LOSS_CFG.MASK_LW > 0 and phase == 'train':
        ...
        if "zoom_x" in batch:
            vis_dict["zoom_img_ren"] = (batch['zoom_x'][i, 0:3].detach().cpu().numpy().transpose(1,2,0) * 255).astype('uint8')[:,:,::-1]
