...
def get_evaluator(cfg, dataset_name, output_folder=None, renderer=None):
    ...
    if evaluator_type == "bop":
        deepim_eval_cls = DeepIM_Evaluator if cfg.VAL.get("USE_BOP", False) else DeepIM_EvaluatorCustom
        return deepim_eval_cls(
            cfg,
            dataset_name,
...
def do_save_results(cfg, model, renderer, epoch=None, iteration=None):
    ...
    for dataset_name in cfg.DATASETS.TEST:
        ...
        data_loader = build_deepim_test_loader(cfg, dataset_name, train_objs=train_obj_names)
        deepim_save_result_of_dataset(
            cfg,
            model,
            renderer,
            data_loader,
...
def do_test(cfg, model, renderer=None, epoch=None, iteration=None):
    results = OrderedDict()
    model_name = osp.basename(cfg.MODEL.WEIGHTS).split(".")[0]
    for dataset_name in cfg.DATASETS.TEST:
        if epoch is not None and iteration is not None:
        ...
        evaluator = get_evaluator(cfg, dataset_name, eval_out_dir, renderer=renderer)

        data_loader = build_deepim_test_loader(cfg, dataset_name, train_objs=evaluator.train_objs)
        results_i = deepim_inference_on_dataset(cfg, model, data_loader, evaluator, amp_test=cfg.TEST.AMP_TEST)
...
def do_train(cfg, args, model, optimizer, renderer, resume=False):
    ...
    data_loader = build_deepim_train_loader(cfg, train_dset_names)
    ...
    if train_2_ratio > 0.0 and len(train_2_dset_names) > 0:
        data_loader_2 = build_deepim_train_loader(cfg, train_2_dset_names)
    ...
    with EventStorage(start_iter) as storage:
        ...
        for iteration in range(start_iter, max_iter):
            ...
            for refine_i in range(1, max_refine_iter + 1):
                # batch update ---------
                batch_updater(cfg, batch, renderer, poses_est=poses_est)
                if cfg.TRAIN.VIS:
