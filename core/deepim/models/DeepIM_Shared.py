...
class DeepIM_Shared(nn.Module):
    def __init__(self, cfg, backbone, pose_head=None, mask_head=None, flow_head=None, renderer=None):
        """use the shared backbone for real/rend."""
        super().__init__()
        assert cfg.MODEL.DEEPIM.NAME == "DeepIM_Shared", cfg.MODEL.DEEPIM.NAME
        self.cfg = cfg
    ...
    def forward(
        ...
        if (self.flow_head is not None) and (do_loss or cfg.TEST.OUTPUT_FLOW):
            flow = self.flow_head(conv_feat)  # assume stride 4
