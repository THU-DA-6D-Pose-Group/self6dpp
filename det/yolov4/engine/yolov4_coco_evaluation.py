...
import ref
from det.yolov4.yolo_utils.utils import clip_coords, cxcywh2xyxy, scale_coords


class YOLOv4COCOEvaluator(DatasetEvaluator):
    ...
    def __init__(self, dataset_name, cfg, distributed, output_dir=None):
        ...
        if "TEST" in cfg:
            self.filter_scene = cfg.TEST.get("FILTER_SCENE", False)
        ...
        if not hasattr(self._metadata, "json_file"):
            ...
            try:
                ...
                self._logger.warning(f"Failed to convert {dataset_name} to COCO format. Do inference only.")
                has_json = False
        if has_json:
        ...
        self._kpt_oks_sigmas = cfg.TEST.get("KEYPOINT_OKS_SIGMAS", [])
    ...
    def process(self, inputs, out_dict):
        """
        Args:
            ...
            out_dict:
        """
        width = out_dict["width"]
        height = out_dict["height"]
        whwh = out_dict["whwh"]
        nms_out = out_dict["nms_out"]
        for si, pred in enumerate(nms_out):
            if pred is None:
                self._logger.warning("No detection for {}".format(inputs[si]["image_id"]))
                continue
            # Clip boxes to image bounds
            clip_coords(pred, (height, width))

            prediction = {"image_id": inputs[si]["image_id"]}

            prediction["instances"] = self.preds_to_coco_json(pred.detach(), inputs[si])

            if "time" in out_dict:
                prediction["time"] = out_dict["time"]
    ...
    def _derive_coco_results(self, coco_eval, iou_type, class_names=None):
        ...
        # tabulate it
        N_COLS = min(10, len(results_per_category) * 2)
    ...
    def preds_to_coco_json(self, preds, input):
        ...
        img = input["image"]
        shapes = input["shapes"]
        scale_coords(img.shape[1:], box, shapes[0], shapes[1])  # to original shape
        ...
        classes = preds[:, 5].cpu().numpy().tolist()
        ...
        for k in range(num_instance):
            if self.filter_scene:
                ...
                scene_id = int(input["scene_im_id"].split("/")[0])
                if scene_id != obj_id:
                    continue
