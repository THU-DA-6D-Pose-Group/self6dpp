...
class LoadImages:  # for inference
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            ...
            self.frame += 1
            iprint(
                "video %g/%g (%g/%g) %s: " % (self.count + 1, self.nf, self.frame, self.nframes, path),
                end="",
            )
            ...
            iprint("image %g/%g %s: " % (self.count, self.nf, path), end="")
...
class LoadWebcam:  # for inference
    ...
    def __next__(self):
        ...
        img_path = "webcam.jpg"
        iprint("webcam %g: " % self.count, end="")
...
class LoadStreams:  # multiple IP or RTSP cameras
    def __init__(self, sources="streams.txt", img_size=640):
        ...
        for i, s in enumerate(sources):
            ...
            iprint(" success (%gx%g at %.2f FPS)." % (w, h, fps))
            thread.start()
        iprint("")  # newline
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(
        ...
        # Get labels
        labels, shapes = zip(*[cache[x] for x in tqdm(self.img_files)])
            ...
            if l.shape[0]:
                ...
                if extract_bounding_boxes:
                    ...
                    for j, x in enumerate(l):
                        ...
                        b[2:] = b[2:] * 1.3 + 30  # pad
                        b = cxcywh2xyxy(b.reshape(-1, 4)).ravel().astype(np.int)
    ...
    def __getitem__(self, index):
        ...
        if self.mosaic:
            ...
            x = self.labels[index]
            if x.size > 0:
                # Normalized cxcywh to pixel xyxy format
                labels = x.copy()
...
def load_mosaic(self, index):
    ...
    for i, index in enumerate(indices):
        ...
        x = self.labels[index]
        labels = x.copy()
        if x.size > 0:  # Normalized cxcywh to pixel xyxy format
...
def reduce_img_size(path="../data/sm4/images", img_size=1024):  # from utils.datasets import *; reduce_img_size()
    ...
    for f in tqdm(glob.glob("%s/*.*" % path)):
        try:
            ...
            iprint("WARNING: image failure %s" % f)
