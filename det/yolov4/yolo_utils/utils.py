...
def check_anchors(dataset, model, thr=4.0, imgsz=640):
    ...
    def metric(k):  # compute metric
        ...
        return (best > 1.0 / thr).float().mean()  #  best possible recall
...
def labels_to_class_weights(labels, nc=80):
    ...
    classes = labels[:, 0].astype(np.int)  # labels = [class cxcywh]
...
def xyxy2cxcywh(x):
    ...
    left, xy2=bottom-right cxcywh means center_xy and wh."""
...
def coco_single_class_labels(path="../coco/labels/train2014/", label_class=43):
    ...
    for file in tqdm(sorted(glob.glob("%s/*.*" % path))):
        ...
        if any(i):
            ...
            shutil.copyfile(
                src=img_file,
                dst="new/images/" + Path(file).name.replace("txt", "jpg"),
            )  # copy images
...
def kmean_anchors(
    ...
    if isinstance(path, str):  # *.yaml file
        ...
        from core.utils import LoadImagesAndLabels
...
def apply_classifier(x, model, img, im0):
    ...
    for i, d in enumerate(x):  # per image
        if d is not None and len(d):
            ...
            b = xyxy2cxcywh(d[:, :4])  # boxes
            b[:, 2:] = b[:, 2:].max(1)[0].unsqueeze(1)  # rectangle to square
            b[:, 2:] = b[:, 2:] * 1.3 + 30  # pad
            d[:, :4] = cxcywh2xyxy(b).long()
...
def plot_images(
    ...
    for i, img in enumerate(images):
        ...
        if len(targets) > 0:
            ...
            boxes = cxcywh2xyxy(image_targets[:, 2:6]).T
