...
parser.add_argument(
    ...
    type=int,
    default=50,
    help="batch size to test image patches in",
)
...
parser.add_argument(
    "--from_scratch",
    action="store_true",
    help="model was initialized from scratch",
)
...
for epoch in range(1, opt.nepoch + opt.nepoch_decay + 1):
    ...
    for i, data in enumerate(dataset):
        ...
        if total_steps % opt.print_freq == 0:
            ...
            t = (time.perf_counter() - iter_start_time) / opt.batch_size
            t2o = (time.perf_counter() - epoch_start_time) / 3600.0
            t2 = t2o * D / (i + 0.0001)
